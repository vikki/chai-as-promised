                    ...
                    foundErrors.forEach(function(foundError){
                        function getErrorMsg(options) {
                            ...
                            typesToErrorMessages = {
                              "differentErrorInstance":     "expected promise " + extra +
                                                            "to be rejected with #{exp} " +
                                                            "but it was fulfilled with #{act}",
                              "differentErrorType":         "expected promise " + extra +
                                                            "to be rejected with #{exp} " +
                                                            "but it was fulfilled with #{act}",
                              "errorMessageDoesNotMatch":   "expected promise " + extra +
                                                            "to be rejected with an error matching #{exp} " +
                                                            "but it was fulfilled with #{act}",
                              "errorMessageDoesInclude":    "expected promise " + extra + "to be rejected " +
                                                            "with an error including #{exp} " +
                                                            "but it was fulfilled with #{act}",
                              "noErrorThrown":              "expected promise to be rejected with #{exp}",
                              "noErrorThrown-name":         "expected promise to be rejected with #{exp} " +
                                                            "but it was fulfilled with #{act}",
                              "noErrorThrown-desiredError": "expected promise to be rejected with " +
                                                            utils.objDisplay(foundError.expected) +
                                                            " but it was fulfilled with #{act}"
                            };
                        ...
                        function getErrorMsg(options) {
                            ...
                            typesToErrorMessages = {
                              ...
                              "errorMessageDoesNotMatch":    "expected promise " + extra +
                                                             "to be rejected with an error matching #{exp} " +
                                                             "but got #{act}",
                              "errorMessageDoesInclude":     "expected promise " + extra +
                                                             "to be rejected with an error including #{exp} " +
                                                             "but got #{act}",
                              "noErrorThrown":               "expected promise to be rejected with #{exp}",
                              "noErrorThrown-name":          "expected promise to be rejected with #{act}",
                              "noErrorThrown-desiredError":  "expected promise to be rejected with " +
                                                             utils.objDisplay(foundError.expected)
                            };
